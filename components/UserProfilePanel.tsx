...
import { X, User, MapPin, Heart, Users, FileText, Edit, Save, Mail, Share, MessageSquare, Download, Eye, CheckCircle, AlertCircle, Phone, Calendar, Shield, Upload, Clock, XCircle, Image, UserCog, AlertTriangle } from 'lucide-react';
...
interface UserData {
  ...
  email: string;
  phone: string;
  userType?: 'titular' | 'dependent' | 'admin';
  plan: string;
...
const UserProfilePanel: React.FC<UserProfilePanelProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<'personal' | 'address' | 'emergency' | 'medical' | 'beneficiaries' | 'documents' | 'contract' | 'user_type'>('documents');
  ...
  const handleSave = async () => {
    ...
    try {
      console.log('Salvando dados atualizados:', editedUser);
      ...
      const { error: userError } = await supabase
        ...
        .update({
          ...
          is_admin: editedUser.userType === 'admin'
        })
        .eq('id', editedUser.id);
      ...
      const { error: personalDataError } = await supabase
        .from('user_profiles')
        .update({
          // Adicionar campos relevantes do perfil
        })
      ...
      // Atualizar endereço
      const addressData = {
        street: editedUser.personalData.address.street,
        number: editedUser.personalData.address.number,
        complement: editedUser.personalData.address.complement,
        neighborhood: editedUser.personalData.address.neighborhood,
        city: editedUser.personalData.address.city,
        state: editedUser.personalData.address.state,
        zip_code: editedUser.personalData.address.zipCode,
        is_primary: true
      };
      ...
      const emergencyContactData = {
        name: editedUser.personalData.emergencyContact.name,
        phone: editedUser.personalData.emergencyContact.phone,
        relationship: editedUser.personalData.emergencyContact.relationship,
        is_primary: true
      };
      ...
      setSaveError(error instanceof Error ? error.message : 'Erro desconhecido ao salvar dados');
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <div className="flex h-[calc(95vh-80px)]">
          ...
          <div className="w-64 bg-gray-50 border-r border-gray-200 p-4">
            <nav className="space-y-2">
              ...
              <button
                onClick={() => setActiveSection('user_type')}
                className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-left transition-colors ${
                  activeSection === 'user_type' ? 'bg-blue-100 text-blue-900' : 'text-gray-700 hover:bg-gray-100'
                }`}
              >
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <DocumentCard
                      ...
                      status={user.documents.titular.proofOfAddress}
                ...
                <div className="bg-purple-50 border border-purple-200 rounded-xl p-6">
                  ...
                  <div className="space-y-4">
                    <div className="mb-4">
                      ...
                      {isEditing ? (
                        <select
                          value={editedUser.userType || 'titular'}
                          onChange={(e) => setEditedUser({...editedUser, userType: e.target.value as 'titular' | 'dependent' | 'admin'})}
                        ...
                        <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                          ...
                          {user.userType === 'admin' ? 'Administrador' :
                           user.userType === 'dependent' ? 'Dependente' : 'Titular'}
                        </div>
                      )}
                    </div>
                      ...
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <p className="text-sm text-yellow-700">
                            Alterar o tipo de usuário pode afetar permissões e acesso ao sistema.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      <h4 className="font-medium text-gray-900 mb-2">Permissões por tipo de usuário:</h4>
                      <ul className="space-y-2 text-sm text-gray-600">
                        <li className="flex items-start space-x-2">
                          <CheckCircle className="h-4 w-4 text-green-600 mt-0.5" />
                          <span><strong>Titular:</strong> Acesso completo aos próprios dados, dependentes e documentos</span>
                        </li>
                        <li className="flex items-start space-x-2">
                          <CheckCircle className="h-4 w-4 text-blue-600 mt-0.5" />
                          <span><strong>Dependente:</strong> Acesso limitado aos próprios dados e documentos</span>
                        </li>
                        <li className="flex items-start space-x-2">
                          <CheckCircle className="h-4 w-4 text-purple-600 mt-0.5" />
                          <span><strong>Administrador:</strong> Acesso completo ao sistema, incluindo painel administrativo</span>
                        </li>
                      </ul>
                    </div>
                  </div>
